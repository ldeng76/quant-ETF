...
Runner->>SSE : broadcast(alert events)
```
...
## 结论
该系统通过“策略执行—结果落盘—告警对比—SSE推送—Dashboard展示”的闭环，实现了分钟级数据驱动的实时监控与告警。告警引擎以规则为核心，结合SSE实现低延迟事件推送；分钟K线采集模块提供高可用行情数据基础；Dashboard侧以SQLite与DuckDB分别承载规则与信号数据，满足可观测性与可扩展性需求。建议在生产环境中强化服务器冷却策略、完善心跳与超时配置，并定期校验数据新鲜度与索引有效性。
