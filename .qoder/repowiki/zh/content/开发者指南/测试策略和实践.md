...
## 更新摘要
**变更内容**
- 新增股票名称刷新机制测试套件章节，详细介绍 `tests/test_name_refresh.py` 的测试策略
- 更新数据源测试章节，增加 `refresh_stock_names` 方法的详细测试分析
- 新增股票名称管理与市场代码推断的测试最佳实践
- 扩展Mock对象使用章节，包含 `SimpleStockAPI.batch_query` 的测试替身设计
...
NS["股票名称刷新<br/>refresh_stock_names"]
SSA["SimpleStockAPI<br/>batch_query"]
END["外部股票API<br/>新浪/腾讯/网易/东财"]
end
subgraph "测试层"
U["单元测试<br/>tests/*.py"]
I["集成测试<br/>tests/*.py"]
...
- [tests/test_name_refresh.py:1-181](file://tests/test_name_refresh.py#L1-L181)
- [src/collect_info/simple_stock_api.py:199-224](file://src/collect_info/simple_stock_api.py#L199-L224)
...
#### 股票名称刷新功能概述
新增的 `tests/test_name_refresh.py` 测试套件专门验证 `ETFDataSource.refresh_stock_names` 方法的可靠性，该方法负责强制全量校准股票代码名称映射文件。

#### 核心测试场景
测试套件覆盖以下关键场景：

1. **错误条目覆盖验证**
   - 验证JSON中错误条目会被在线API返回值覆盖
   - 测试5/6开头代码的市场字段统一修正为"sh"
   - 确保0/1/3开头代码的市场字段统一修正为"sz"

2. **本地市场推断机制**
   - 验证市场字段必须由 `_market_for_code` 本地推断，忽略API错误的市场信息
   - 测试各种代码前缀的正确市场判定

3. **失败查询处理**
   - 在线查询失败时保留旧条目，并加入failed列表
   - 确保失败代码不会被覆盖

4. **干运行模式**
   - `dry_run=True` 时不应写入文件，但报告仍显示差异

#### Mock策略与测试替身
- 使用 `monkeypatch` 打桩配置常量，避免依赖真实配置文件
- 使用 `unittest.mock.patch` 替换 `SimpleStockAPI.batch_query` 方法
- 通过 `tmp_path` 创建临时文件系统，确保测试隔离

**章节来源**
- [tests/test_name_refresh.py:1-181](file://tests/test_name_refresh.py#L1-L181)
- [src/quant_etf/data_source.py:346-463](file://src/quant_etf/data_source.py#L346-L463)
- [src/quant_etf/cli.py:379-402](file://src/quant_etf/cli.py#L379-L402)
...
- [tests/test_name_refresh.py:18-35](file://tests/test_name_refresh.py#L18-L35)
...
- 常见问题与解决
  ...
  - 股票名称刷新失败：检查API可用性和网络连接
  - 市场代码推断错误：验证代码前缀规则和补零逻辑
- 调试建议
  - 使用pytest --pdb进入交互式调试
...
## 结论
本测试策略以pytest为核心，结合单元、集成与端到端三层测试，配合Mock与共享fixtures，既保证了测试的可重复性与稳定性，又覆盖了真实业务场景。新增的股票名称刷新测试套件进一步完善了测试体系，确保数据源模块的关键功能得到充分验证。通过markers与跳过机制适配不同环境，借助覆盖率与CI流程保障质量门槛。建议在后续迭代中逐步完善覆盖率阈值与报告归档，持续优化测试执行效率与可维护性。
...
- 快速参考
  ...
  - 股票名称刷新：refresh_stock_names方法、市场代码推断、干运行模式
